...
int main( int argc, char* argv[] ) {
  ...
  if( argc!= 5 ) {
    std::cout << "USAGE: ./makeSpectrum [rootFileName] [channel] [xMinFit] [xMaxFit] " << std::endl;
    exit(1);
  }

  std::string fileName(argv[1]);
  int channel(atoi(argv[2]));
  double xMinFit(atof(argv[3])), xMaxFit(atof(argv[4]));
  ...
  TF1 *f1 = new TF1("f1", "[0]+exp(-(x-[1])*(x-[1])/(2*[2]*[2]))", xMinFit, xMaxFit );
  //f1->SetParameters(0, 40, 5);
  h_charge->Fit("f1");
  h_charge->Write();
